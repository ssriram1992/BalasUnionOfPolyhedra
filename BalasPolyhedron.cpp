		...
		)
{
	...
	for (unsigned int i=0; i!=N_poly; i++)					// Iterating over each polyhedron
	{
		for(unsigned int k=0; k!=N_vars; k++)				// Adding variables to the model
			x[i][k] = model.addVar(LB, UB, 0, GRB_CONTINUOUS, "x_"+to_string(i)+"_"+to_string(k));
	...
	{
		xMain[k] = model.addVar(-GRB_INFINITY, GRB_INFINITY, 1, GRB_CONTINUOUS, "xMain_"+to_string(k));
		...
		)
{
	...
	for (unsigned int k=0; k!= N_vars; k++)					// Adding the actual variables
	{
		xMain[k] = model.addVar(-GRB_INFINITY, GRB_INFINITY, 0, GRB_CONTINUOUS, "xMain_"+to_string(k));
		lincons = GRBLinExpr{0};
		for (unsigned int i=0; i!=N_poly; i++)
			lincons+=x[i][k];
		lincons -= xMain[k];
